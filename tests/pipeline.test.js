...
import { Store, loadTransactions, putMany } from '../static/js/data/client.js';
import { catCache, primeCategoryCache, resolveCategory } from '../static/js/data/categories.js';
import { backfill, catchUp } from '../static/js/data/ingest.js';

const Api = { Store, loadTransactions, putMany, catCache, primeCategoryCache, resolveCategory, backfill, catchUp };
...
function buildInbox() {
  ...
  // Cross-sender duplicate: the SAME payment reported by the UPI app, seconds
  // later, with no ref and no account number.
  //
  // Day 3 of the loop above is AMAZON for 100 + 3*37 = 211, so the copy must
  // name AMAZON too. The original fixture said "Uber", which made this a pair
  // of UNRELATED payments -- and it passed only because the old soft key
  // ignored the merchant and merged them. The fixture was the bug.
  const dupDate = BASE + 3 * DAY;
  msgs.push({ id: id++, date: dupDate + 40000, address: 'JM-PAYTM',
    body: 'You paid Rs.211.00 to AMAZON via Paytm UPI on 04-07-25' });
...
(async () => {
  ...
  // ============ 7. stale cursor must not silently truncate a run ============
  //
  // A cursor left behind by a completed run pointed at the OLDEST message, so
  // every later "Start import" resumed at the end of history, read a handful of
  // messages and reported success. Reproduced as: 120 in the inbox, 3 scanned.
  {
    store.clear();
    const oldest = Math.min(...INBOX.map((m) => m.date));
    ls.set('sandeshika.cursor', String(oldest + 1));

    const stranded = await Api.backfill({ shouldStop: () => false });
    ok('stale cursor does truncate a plain run', stranded.scanned < INBOX.length,
       `${stranded.scanned}/${INBOX.length}`);
    ok('the run reports where it resumed from', stranded.resumedFrom === oldest + 1);

    const full = await Api.backfill({ shouldStop: () => false, restart: true });
    ok('restart:true reads the whole inbox', full.scanned === INBOX.length,
       `${full.scanned}/${INBOX.length}`);
    ok('restart reports completion', full.complete === true);
    ok('cursor cleared once history is exhausted', !Api.Store.cursor(),
       String(Api.Store.cursor()));

    // and the very next plain run must not be stranded again
    const after = await Api.backfill({ shouldStop: () => false });
    ok('a completed run leaves the next one un-stranded', after.scanned === INBOX.length,
       `${after.scanned}/${INBOX.length}`);
    ok('re-run still writes nothing new', after.written === 0, String(after.written));
  }


  // ====== 8. a re-scan must RE-PARSE stored rows, not skip them ======
  //
  // Skipping was the old behaviour, and it meant every parser improvement was
  // invisible on data already imported: transfers stayed classified as
  // spending and months of totals stayed wrong however often the user
  // re-imported. User decisions must still survive the re-parse.
  {
    store.clear(); ls.delete('sandeshika.cursor'); ls.delete('sandeshika.high');
    await Api.backfill({ shouldStop: () => false, restart: true });

    let rows = await Api.loadTransactions();
    const victim = rows.find((t) => t.kind === 'expense' && t.merchant);
    ok('there is a row to corrupt', !!victim);

    // Simulate a row stored by an older, worse parser.
    await Api.putMany([{ key: 'txn/' + victim.fingerprint,
      value: JSON.stringify({ ...victim, kind: 'income', category: 'other',
                              categorySource: 'llm', merchant: 'WRONG' }) }]);

    // And one the user fixed by hand, which must NOT be overwritten.
    const keeper = rows.find((t) => t.fingerprint !== victim.fingerprint && t.merchant);
    await Api.putMany([{ key: 'txn/' + keeper.fingerprint,
      value: JSON.stringify({ ...keeper, category: 'education',
                              categorySource: 'user', kind: 'transfer',
                              kindSource: 'user', reviewed: true }) }]);

    const res = await Api.backfill({ shouldStop: () => false, restart: true });
    ok('the re-scan reports updates', res.updated > 0, String(res.updated));

    rows = await Api.loadTransactions();
    const fixed = rows.find((t) => t.fingerprint === victim.fingerprint);
    ok('a stale row is re-parsed', fixed.kind === 'expense' && fixed.merchant !== 'WRONG',
       `${fixed.kind} / ${fixed.merchant}`);

    const kept = rows.find((t) => t.fingerprint === keeper.fingerprint);
    ok('a user category survives the re-parse', kept.category === 'education', kept.category);
    ok('a user kind survives the re-parse', kept.kind === 'transfer', kept.kind);

    // A plain resume must not rewrite anything.
    const plain = await Api.backfill({ shouldStop: () => false });
    ok('a normal run rewrites nothing', (plain.updated || 0) === 0, String(plain.updated));
  }
